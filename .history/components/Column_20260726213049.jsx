...
import { LuPencil, LuTrash, LuGrip } from 'react-icons/lu';
export default function Column({
  ...
  return (
    <div className='w-80 flex-shrink-0 bg-slate-50 dark:bg-gray-900 border border-slate-200 dark:border-gray-700 rounded-2xl p-4'>
      <div className='flex items-center justify-between mb-4'>
        <div className='flex items-center'>
          ...
          {isEditing ? (
            ...
            <h2 className='text-xl font-semibold text-slate-600 dark:text-gray-100'>{column.title}</h2>
          )}
        </div>
        <div className='flex space-x-2'>
          {isEditing ? (
            ...
            <button onClick={() => setIsEditing(true)} className='text-slate-500 dark:text-gray-400 hover:text-slate-700 dark:hover:text-gray-200 transition-colors'>
              <LuPencil size={20} />
            </button>
          )}
          <button onClick={() => onDeleteColumn(column.id)} className='text-red-600 hover:text-red-800 transition-colors'>
            <LuTrash size={20} />
          </button>
        </div>
      </div>
