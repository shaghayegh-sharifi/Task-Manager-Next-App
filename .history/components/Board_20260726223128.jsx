...
export default function Board() {
  ...
  const handleDragEnd = (event) => {
    ...
    if (sourceColumnId && targetColumnId) {
      if (sourceColumnId === targetColumnId) {
        ...
        const newTasks = arrayMove(data.columns[sourceColumnId].tasks, activeTaskIndex, overTaskIndex);
        setData({
          ...data,
          columns: {
            ...data.columns,
  ...
  const handleEditTask = (columnId, taskId, newContent) => {
    const newTasks = data.columns[columnId].tasks.map((task) => (task.id === taskId ? { ...task, content: newContent } : task));
    setData({
      ...data,
      columns: {
        ...data.columns,
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <SortableContext items={data.columnOrder} strategy={horizontalListSortingStrategy}>
        <div className='flex flex-1 flex-nowrap overflow-x-auto p-6 space-x-6 border rounded-3xl bg-indigo-50 dark:bg-indigo-900 dark:border-indigo-700 rounded-3xl shadow-md'>
          ...
          {/* Add Column UI */}
          <div className='w-80 h-max bg-indigo-50 dark:bg-indigo-900 border border-indigo-200 dark:border-indigo-700 rounded-3xl p-4 shadow-md'>
            <h2 className='text-xl font-semibold text-indigo-800 dark:text-indigo-200 mb-4'>Add Column</h2>
            <input
              type='text'
              placeholder='Column title...'
              value={newColumnTitle}
              onChange={(e) => setNewColumnTitle(e.target.value)}
              className='w-full border border-indigo-200 dark:border-indigo-600 p-2 rounded-2xl focus:outline-none focus:ring focus:ring-indigo-300 dark:focus:ring-indigo-500 mb-4 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100'
            />
            <button onClick={handleAddColumn} className='w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded-2xl transition-colors'>
              Add Column
            </button>
          </div>
        </div>
      ...
      <DragOverlay>
        {activeTask ? (
          <div className='p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md shadow-lg'>
            <span className='text-slate-800 dark:text-slate-100'>{activeTask.content}</span>
          </div>
        ) : null}
      </DragOverlay>
    </DndContext>
