...
export default function Task({ task, columnId, onDeleteTask, onEditTask }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className='p-3 bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-2xl shadow-sm mb-2 flex items-center transition-transform duration-150 hover:shadow-lg'>
      {/* Drag handle */}
      <div {...attributes} {...listeners} className='cursor-move mr-3 text-slate-500 dark:text-gray-400'>
        <LuGripHorizontal />
      </div>
      {/* Task content */}
      ...
      <div className='flex space-x-2 ml-3'>
        {isEditing ? (
          ...
          <button
            ...
            className='bg-blue-600 hover:bg-blue-700 text-white transition-colors border p-1.5 rounded-full'>
            <LuPencil />
          </button>
        )}
        <button
